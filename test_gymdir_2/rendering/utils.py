"""
WARNING: as member of directory pddlgym/rendering, implicitly depends upon pddlgym/rendering/__init__.py, which is NOT position-independent
- Do NOT import this file if operating with non-default $CURRENT_GYMDIR
- Instead, copy this file to $CURRENT_GYMDIR/rendering and remove this warning text
"""
...
DEFAULT_RENDERINGDIR = os.path.dirname(os.path.realpath(__file__))  # i.e. subdir `rendering` of DEFAULT_GYMDIR

# get env var CURRENT_GYM if it exists; otherwise, use the default
def RENDERING_current_gymdir():
    """
    Namespace-unique against pddlgym/__init__.py:current_gymdir()
    """
    _current_gymdir = os.getenv("CURRENT_GYMDIR")
    assert _current_gymdir is not None
    return os.path.realpath(_current_gymdir)

def current_renderingdir():
    """
    Although pddlgym/rendering/utils.py will never move,
    this function allows this file to be copied as-is to non-default gymdirs
    """
    if os.getenv("CURRENT_GYMDIR") is None:
        return DEFAULT_RENDERINGDIR
    else:
        return os.path.join(RENDERING_current_gymdir(), "rendering")

def get_asset_path(asset_name):
    dir_path = current_renderingdir()
